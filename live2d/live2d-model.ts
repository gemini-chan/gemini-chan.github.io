...
export class Live2DModelComponent extends LitElement {
  ...
  private _startLoop() {
    ...
    const cb = () => {
      ...
      try {
        ...
        if (
          this.motionName &&
          this.motionName !== this._lastMotionNameApplied
        ) {
  ...
  private _extractEmotionNames(model: Live2DModelLike): string[] {
    ...
    const motionGroups =
      ...
      'motionGroups' in motionManager
        ? (motionManager as { motionGroups?: Record<string, unknown> })
            .motionGroups
        : undefined
    if (!motionGroups) return []
    return Object.keys(motionGroups).filter(
      (name) => name !== 'idle' && name !== 'tap'
    )
  }
  ...
  public setEmotion(emotionName: string) {
    ...
    try {
      ...
      log.warn('setEmotion failed to start motion', {
        emotion: emotionName,
        error: e,
      })
    }
  ...
  private _applyPlacement(modelArg?: Live2DModelLike) {
    ...
    try {
      ;(
        m.position as unknown as { set: (x: number, y: number) => void }
      )?.set?.(cw / 2 + (this.xOffset || 0), ch + (this.yOffset || 0))
    } catch {}
  }
}
