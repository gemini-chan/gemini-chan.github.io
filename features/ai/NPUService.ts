...
export class NPUService {
  ...
  private static readonly PERMANENCE_RANK: Record<string, number> = {
    permanent: 2,
    contextual: 1,
    temporary: 0,
  }
  ...
  public async analyzeAndAdvise(
    ...
    let memories: Memory[] = []
    await this._sendProgress(progressCb, {
      type: 'npu:memories:start',
      ts: Date.now(),
      data: { turnId },
    })
    try {
      memories = await this.memoryService.retrieveRelevantMemories(
        userInput,
        personaId,
        5,
        { emotionBias }
      ...
      logger.error('Failed to retrieve memories for analyzeAndAdvise', {
        error,
        personaId,
      })
    }
    ...
    // Build prompt
    logger.debug('analyzeAndAdvise: memories retrieved', {
      count: memories.length,
    })
    await this._sendProgress(progressCb, {
      type: 'npu:memories:done',
      ts: Date.now(),
      data: { count: memories.length, memories, turnId },
    })

    // Rank and limit memories before building context
    const rankedMemories = this._rankAndLimitMemories(
      memories,
      NPUService.MAX_MEMORY_LINES
    )

    // Build memory context string from retrieved memories with partial progress
    const memoryLines = rankedMemories.map(
      (m) =>
        `- ${m.fact_key}: ${m.fact_value} (conf=${(m.confidence_score ?? 0).toFixed(2)}, perm=${m.permanence_score})`
    )
    for (const line of memoryLines) {
      // No delay for partials
      progressCb?.({
        type: 'npu:prompt:partial',
        ts: Date.now(),
        data: { delta: line + '\n', turnId },
      })
    }
    ...
    await this._sendProgress(progressCb, {
      type: 'npu:prompt:build',
      ts: Date.now(),
      data: { turnId },
    })
    const rtStr =
      typeof localStorage !== 'undefined'
        ? localStorage.getItem(NPU_STORAGE_KEYS.recentTurns)
        : null
    ...
    const combinedPromptText = this.buildCombinedPrompt(
      userInput,
      limitedMemory,
      limitedConversation
    )
    await this._sendProgress(progressCb, {
      type: 'npu:prompt:built',
      ts: Date.now(),
      data: {
        promptPreview: combinedPromptText.slice(0, 500),
        fullPrompt: combinedPromptText,
        turnId,
      },
    })
    logger.debug('analyzeAndAdvise: combined prompt built', {
      length: combinedPromptText.length,
      memoryLines: memories.length,
    })
    ...
    const model =
      (typeof localStorage !== 'undefined'
        ? localStorage.getItem(NPU_STORAGE_KEYS.model)
        : null) || NPU_DEFAULTS.model
    const tempStr =
      typeof localStorage !== 'undefined'
        ? localStorage.getItem(NPU_STORAGE_KEYS.temperature)
        : null
    const storedThinking =
      (typeof localStorage !== 'undefined'
        ? localStorage.getItem(NPU_STORAGE_KEYS.thinkingLevel)
        : null) || NPU_DEFAULTS.thinkingLevel
    let temperature: number = NPU_DEFAULTS.temperature
    if (
      tempStr !== null &&
      tempStr !== '' &&
      !Number.isNaN(parseFloat(tempStr))
    ) {
    ...
    const topPStr =
      typeof localStorage !== 'undefined'
        ? localStorage.getItem(NPU_STORAGE_KEYS.topP)
        : null
    ...
    const topKStr =
      typeof localStorage !== 'undefined'
        ? localStorage.getItem(NPU_STORAGE_KEYS.topK)
        : null
    ...
    const thinkingLevel = (
      Object.keys(NPU_THINKING_TOKENS).includes(storedThinking)
        ? storedThinking
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (turnId && this.activeTurnId && this.activeTurnId !== turnId) {
        await this._sendProgress(
          progressCb,
          {
            type: 'npu:stale',
            ts: Date.now(),
            data: { turnId, active: this.activeTurnId },
          },
          0
        )
        responseText = ''
      ...
      try {
        ...
        await this._sendProgress(progressCb, {
          type: 'npu:model:response',
          ts: Date.now(),
          data: { length: responseText.length, turnId },
        })
        logger.debug('analyzeAndAdvise: model responded', {
        ...
        if (attempt < maxAttempts) {
          await new Promise((res) =>
            setTimeout(res, Math.pow(2, attempt) * 200)
          )
          continue
        }
      }
    ...
    await this._sendProgress(progressCb, {
      type: 'npu:advisor:ready',
      ts: Date.now(),
      data: { length: advisorContext.length, turnId },
    })
    ...
    progressCb?.({
      type: 'npu:complete',
      ts: Date.now(),
      data: { hasResponseText: !!responseText.length, turnId },
    })
  ...
  private async _withTimeout<T>(
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      timer = setTimeout(
        () => reject(new Error(`timeout: ${label || 'op'}`)),
        ms
      )
    })
    try {
  ...
  private _truncate(
    s: string | undefined,
    max = NPUService.MAX_CONTEXT_CHARS
  ): string {
    if (!s) return ''
