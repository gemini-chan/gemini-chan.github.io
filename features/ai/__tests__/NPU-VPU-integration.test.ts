...
describe("NPU-VPU Integration", () => {
  ...
  it("should preserve original message and add memory context", async () => {
    ...
    expect(result.enhancedPrompt).to.include(
  ...
  it("should handle cases with no relevant memories", async () => {
    ...
    const emptyNpuService = new NPUService(
      mockAIClient as any,
      mockEmptyMemoryService as any,
    );

    const userMessage = "What is the weather today?";
    const personaId = "test-persona";

    const result = await emptyNpuService.createRAGPrompt(
      userMessage,
      personaId,
    );
  ...
  it("should gracefully handle NPU failures", async () => {
    ...
    const failingNpuService = new NPUService(
      mockFailingAIClient as any,
    ...
    const result = await failingNpuService.createRAGPrompt(
      userMessage,
      personaId,
    );
