...
function mkMemoryService(opts?: { throw?: boolean; memories?: unknown[] }): IMemoryService {
  return {
    retrieveRelevantMemories: vi.fn(async () => {
      ...
      return (
        opts?.memories || [
          { fact_key: 'name', fact_value: 'Airi', confidence_score: 0.9, permanence_score: 'permanent', timestamp: new Date(), conversation_turn: 't1', personaId: 'p1' },
          { fact_key: 'likes', fact_value: 'ramen', confidence_score: 0.8, permanence_score: 'contextual', timestamp: new Date(), conversation_turn: 't2', personaId: 'p1' },
        ]
      );
    }),
...
describe('NPUService hardening', () => {
  ...
  it('falls back to defaults on invalid localStorage values and uses thinking tokens', async () => {
    ...
    expect(call.generationConfig.maxOutputTokens).toBe(NPU_THINKING_TOKENS['standard']);
  });
  ...
  it('uses provided conversationContext instead of recent transcript when present', async () => {
    ...
    const payload = await svc.analyzeAndAdvise('Hello', 'p1', transcript, 'SPECIAL_CONTEXT');
    expect(payload.advisor_context).toBe('ok');
  ...
  it('ranks and limits memories: permanent > contextual > temporary; confidence desc', async () => {
    const memories = [
      { fact_key: 'temp_low', fact_value: 'temp', confidence_score: 0.5, permanence_score: 'temporary', timestamp: new Date(), conversation_turn: 't1', personaId: 'p1' },
      { fact_key: 'temp_high', fact_value: 'temp', confidence_score: 0.9, permanence_score: 'temporary', timestamp: new Date(), conversation_turn: 't2', personaId: 'p1' },
      { fact_key: 'context_low', fact_value: 'context', confidence_score: 0.4, permanence_score: 'contextual', timestamp: new Date(), conversation_turn: 't3', personaId: 'p1' },
      { fact_key: 'context_high', fact_value: 'context', confidence_score: 0.8, permanence_score: 'contextual', timestamp: new Date(), conversation_turn: 't4', personaId: 'p1' },
