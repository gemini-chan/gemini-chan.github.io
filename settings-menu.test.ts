...
vi.mock('./src/persona-manager', () => {
  const mockPersonas: Persona[] = [
    { id: 'default-1', name: 'Gemini-chan', systemPrompt: 'Default Prompt', live2dModelUrl: '', isDefault: true },
    { id: 'custom-1', name: 'Custom Persona', systemPrompt: 'Custom Prompt', live2dModelUrl: '', isDefault: false },
  ];
  ...
  const PersonaManagerMock = vi.fn(() => ({
    ...
    createPersona: vi.fn((name: string) => {
        const newPersona = { id: `new-${Math.random()}`, name, systemPrompt: '', live2dModelUrl: '', isDefault: false };
...
describe('SettingsMenu Persona Management', () => {
  ...
  it('should render the list of personas on load', () => {
    const personaButtons = element.shadowRoot!.querySelectorAll('.persona-button');
  ...
  it('should highlight the active persona', () => {
    const activeButton = element.shadowRoot!.querySelector('.persona-button.active');
  ...
  it('should call setActivePersona and update the UI when a persona is selected', async () => {
    ...
    vi.mocked(personaManager.getActivePersona).mockReturnValue(personaManager.getPersonas()[1]);
    ...
    await waitUntil(
      () => element.shadowRoot!.querySelector('.persona-button.active')?.textContent?.trim() === 'Custom Persona',
  ...
  it('should call createPersona when the create button is clicked', async () => {
    const createButton = element.shadowRoot!.querySelector('.section-header button') as HTMLElement;
    createButton.click();
    await element.updateComplete;
  ...
  it('should show the editor for the new persona after creation', async () => {
    const newPersona = { id: 'new-1', name: 'New Persona', systemPrompt: '', live2dModelUrl: '', isDefault: false };
    vi.mocked(personaManager.createPersona).mockReturnValue(newPersona);
    vi.mocked(personaManager.getPersonas).mockReturnValue([...personaManager.getPersonas(), newPersona]);
    ...
    const editor = element.shadowRoot!.querySelector('.persona-editor');
    expect(editor).toBeInTheDocument();
    const nameInput = editor!.querySelector('input[type="text"]') as HTMLInputElement;
  ...
  it('should call updatePersona when the save button is clicked', async () => {
    ...
    const customPersonaButton = element.shadowRoot!.querySelectorAll('.persona-button')[1] as HTMLElement;
    customPersonaButton.click();
    await element.updateComplete;
    ...
    const updatedPersonaArg = vi.mocked(personaManager.updatePersona).mock.calls[0][0];
