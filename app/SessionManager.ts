...
export class SessionManager {
  ...
  public messageStatuses: Record<
    string,
    'clock' | 'single' | 'double' | 'error'
  > = {}
  ...
  public async initTextSession(): Promise<boolean> {
    ...
    if (
      ...
      this.deps.hostElement.dispatchEvent(
        new CustomEvent('status-update', {
          detail: { message: 'Initializing text session...' },
          bubbles: true,
          composed: true,
        })
      )
  ...
  public async initCallSession() {
    this.deps.hostElement.dispatchEvent(
      new CustomEvent('status-update', {
        detail: { message: 'Initializing call session...' },
        bubbles: true,
        composed: true,
      })
    )
    ...
    if (!ok || !this.deps.callSessionManager.sessionInstance) {
      ...
      const exhausted = this.deps.energyBarService.getCurrentEnergyLevel() === 0
      if (exhausted) {
        this.deps.hostElement.dispatchEvent(
          new CustomEvent('status-update', {
            detail: { message: 'Energy exhausted — please try again later.' },
            bubbles: true,
            composed: true,
          })
        )
        ...
        this.deps.hostElement.dispatchEvent(
          new CustomEvent('status-update', {
            detail: {
              message: 'Unable to start call (rate limited or network error)',
            },
            bubbles: true,
            composed: true,
          })
  ...
  public resetTextContext() {
    ...
    this.deps.hostElement.dispatchEvent(
      new CustomEvent('status-update', {
        detail: { message: 'Text conversation cleared.' },
        bubbles: true,
        composed: true,
      })
    )
  ...
  public resetCallContext() {
    ...
    const callT = this.deps.queryShadowRoot(
      'call-transcript'
    ) as HTMLElement & {
      rateLimited?: boolean
    }
    ...
    this.deps.hostElement.dispatchEvent(
      new CustomEvent('status-update', {
        detail: { message: 'Call conversation cleared.' },
        bubbles: true,
        composed: true,
      })
    )
