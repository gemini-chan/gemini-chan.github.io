...
export class GdmLiveAudio extends LitElement {
  ...
  private vpuTranscriptionAgg = new Map<
    string,
    ...
    if (this.sessionManager.textTranscript.length > 0) {
      const lastMessage =
        this.sessionManager.textTranscript[
          this.sessionManager.textTranscript.length - 1
        ]
  ...
  private updateCallTranscript(text: string, speaker: 'user' | 'model') {
    ...
    const lastTurn =
      this.sessionManager.callTranscript[
        this.sessionManager.callTranscript.length - 1
      ]
  ...
  private async _handleCallStart() {
    try {
      ...
      logger.debug(
        'Call start. Existing callSession:',
        this.sessionManager.callSession
      )
  ...
  private _handleCallEnd = async () => {
    if (!this.sessionManager) return
    if (!this.isCallActive && !this.audioManager.mediaStream) return
    ...
    this.callState = 'idle'
    logger.debug(
      'Call ended. callSession preserved:',
      this.sessionManager.callSession
    )
  }
    ...
    if (this.ttsCaption.trim()) {
      ...
      if (lastUserTurn) {
        ...
        this.memoryService
          ...
          .catch((error) => {
            logger.error('Failed to extract and store facts in background', {
              error,
            })
          })
      }
  ...
  private async _startTtsFromSummary(e: CustomEvent) {
    ...
    const { turnId, newTranscript, newStatuses } =
      this.turnManager.initializeNewTurn(
        message,
        this.sessionManager.textTranscript,
        this.sessionManager.messageStatuses
      )
    ...
    logger.debug('UI INIT: Thinking set pre-await', {
      status: this.turnManager.npuStatus,
      active: this.turnManager.thinkingActive,
    })
    ...
    const vpuPayload = this.sessionManager.constructVpuMessagePayload(
      intention?.advisor_context || '',
      message
    )
  ...
  private _handleNpuProgress(ev: NpuProgressEvent, turnId: string) {
    ...
    if (
      this.turnManager.turnState.id === turnId &&
      (this.turnManager.turnState.phase === 'complete' ||
        this.turnManager.turnState.phase === 'error')
    ) {
      return
    }
    // Ignore events for other turns - with type safety check
    if (
      this.turnManager.turnState.id &&
      ev.data?.turnId &&
    ...
    const eventTurnId =
      (ev.data?.turnId as string) || this.turnManager.turnState.id
    ...
    switch (ev.type) {
      ...
      case 'npu:model:start':
        if (ev.data?.model) {
          ...
          this.sessionManager.messageRetryCount = {
            ...this.sessionManager.messageRetryCount,
            [turnId]: 0,
          }
      ...
      case 'npu:model:attempt':
        if (ev.data?.attempt) {
          ...
          this.sessionManager.messageRetryCount = {
            ...this.sessionManager.messageRetryCount,
            [turnId]: Math.max(
              this.sessionManager.messageRetryCount[turnId] ?? 0,
              retries
            ),
          }
  ...
  public _armDevRaf() {
    ...
    const loop = () => {
      ...
      if (
        this.turnManager.turnState.phase === 'vpu' &&
        this.turnManager.vpuHardDeadline > 0
      ) {
        const now = Date.now()
  ...
  private _handleVpuProgress(
    ...
    if (
      ev.type === 'vpu:response:complete' ||
      ev.type === 'vpu:message:error'
    ) {
      if (eventTurnId) {
      ...
      case 'vpu:response:transcription':
        ...
        if (ev.data?.text) {
          ...
          if (now - agg.last > 1000) {
            logger.debug(
              `VPU received ${agg.count} transcription chunks for turn ${turnKey} in the last second.`
            )
            agg.count = 0
            agg.last = now
          }
  ...
  connectedCallback() {
    ...
    window.addEventListener('persona-changed', this._handlePersonaChanged)
    ...
    debugLogger.setThrottle('ChatView', 1000, {
      leading: true,
      trailing: false,
    })
    debugLogger.setThrottle('transcript-auto-scroll', 1000, {
      leading: true,
      trailing: false,
    })
    debugLogger.setThrottle('call-transcript', 1000, {
      leading: true,
      trailing: false,
    })
  ...
  disconnectedCallback() {
    ...
    window.removeEventListener('persona-changed', this._handlePersonaChanged)
    ...
    this.removeEventListener(
      'turn-start',
      this.handleTurnStart as EventListener
    )
  ...
  private startEmotionAnalysis() {
    ...
    this.emotionAnalysisTimer = window.setInterval(async () => {
      ...
      const newTurns = transcript.slice(
        this.sessionManager.lastAnalyzedTranscriptIndex
      )
      ...
      const modelEmotion =
        this.memoryService?.getLastModelEmotion?.() || 'neutral'
      if (modelEmotion !== this.currentEmotion) {
        ...
        this.dispatchEvent(
          new CustomEvent('model-emotion-change', {
  ...
  private _handleReconnecting = () => {
    ...
    if (
      this.activeMode === 'calling' &&
      !this.sessionManager._callReconnectingNotified
    ) {
  ...
  render() {
    return html`
      ...
      <div class="ui-grid">
        <div
          class="main-container ${this.activeMode === 'calling'
            ? 'hidden'
            : ''}"
        >
          <tab-view
