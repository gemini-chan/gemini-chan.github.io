...
export class AudioManager {
  ...
  public updateActiveOutputNode() {
    ...
    this.deps.hostElement.dispatchEvent(
      new CustomEvent('state-update', { bubbles: true, composed: true })
    )
  }
  ...
  public async startAudioProcessing() {
    ...
    try {
      ...
      this.deps.hostElement.dispatchEvent(
        new CustomEvent('error-update', {
          detail: { message: 'Call session failed to become ready in time.' },
          bubbles: true,
          composed: true,
        })
      )
    ...
    this.audioWorkletNode.port.onmessage = (event) => {
      ...
      try {
        ...
        this.deps.hostElement.dispatchEvent(
          new CustomEvent('error-update', {
            detail: { message: `Failed to stream audio: ${msg}` },
            bubbles: true,
            composed: true,
          })
        )
