...
export class HealthMetricsService {
  ...
  public timeNPUProcessing(): () => void {
    ...
    return () => {
      ...
      logger.debug('NPU processing time', {
  ...
  public timeVPUStart(): () => void {
    ...
    return () => {
      ...
      logger.debug('VPU start latency', {
        duration: `${duration.toFixed(2)}ms`,
      })
    }
  }
  ...
  public timeMPUProcessing(): () => void {
    ...
    return () => {
      ...
      logger.debug('MPU processing time', {
        duration: `${duration.toFixed(2)}ms`,
      })
    }
  }
  ...
  public getAverageMetrics(): Partial<HealthMetrics> {
    ...
    const npuLatencies = this.metrics
      .filter((m) => m.npuLatency !== undefined)
      .map((m) => m.npuLatency!)
    const vpuLatencies = this.metrics
      .filter((m) => m.vpuStartLatency !== undefined)
      .map((m) => m.vpuStartLatency!)
    const mpuTurnarounds = this.metrics
      .filter((m) => m.mpuTurnaround !== undefined)
      .map((m) => m.mpuTurnaround!)

    return {
      npuLatency:
        npuLatencies.length > 0
          ? npuLatencies.reduce((a, b) => a + b, 0) / npuLatencies.length
          : undefined,
      vpuStartLatency:
        vpuLatencies.length > 0
          ? vpuLatencies.reduce((a, b) => a + b, 0) / vpuLatencies.length
          : undefined,
      mpuTurnaround:
        mpuTurnarounds.length > 0
          ? mpuTurnarounds.reduce((a, b) => a + b, 0) / mpuTurnarounds.length
          : undefined,
      timestamp: Date.now(),
    }
  }
