...
describe('PersonaManager', () => {
  ...
  beforeEach(() => {
    ...
    localStorageMock = {};
    vi.spyOn(Storage.prototype, 'getItem').mockImplementation((key: string) => localStorageMock[key]);
  ...
  it('should load existing personas from localStorage', () => {
    const existingPersonas: Persona[] = [
      { id: '1', name: 'Test Persona 1', systemPrompt: 'Prompt 1', live2dModelUrl: '', isDefault: true },
      { id: '2', name: 'Test Persona 2', systemPrompt: 'Prompt 2', live2dModelUrl: '', isDefault: false },
    ];
  ...
  describe('getActivePersona', () => {
    ...
    it('should return the default persona if active ID is not found', () => {
      ...
      const activePersona = personaManager.getActivePersona();
      const defaultPersona = personaManager.getPersonas().find(p => p.isDefault);
  ...
  describe('setActivePersona', () => {
    it('should set the active persona ID in localStorage', () => {
      ...
      expect(localStorageMock['geminichan-active-persona-id']).toBe(newPersona.id);
    });
    ...
    it('should log an error if persona ID does not exist', () => {
      ...
      expect(localStorageMock['geminichan-active-persona-id']).not.toBe(nonExistentId);
      expect(consoleErrorSpy).toHaveBeenCalledWith(`Persona with ID "${nonExistentId}" not found.`);
  ...
  describe('createPersona', () => {
    ...
    it('should save the updated persona list to localStorage', () => {
        ...
        const storedPersonas = JSON.parse(localStorageMock['geminichan-personas']);
        expect(storedPersonas.some((p: Persona) => p.name === personaName)).toBe(true);
    });
  });
  ...
  describe('updatePersona', () => {
    ...
    it('should not allow changing the isDefault status of the default persona', () => {
      ...
      const updatedDefaultPersona: Persona = { ...defaultPersona, isDefault: false };

      personaManager.updatePersona(updatedDefaultPersona);
      
      const personas = personaManager.getPersonas();
      const stillDefaultPersona = personas.find(p => p.id === defaultPersona.id);
      expect(stillDefaultPersona?.isDefault).toBe(true);
      expect(consoleWarnSpy).toHaveBeenCalledWith('Cannot change the isDefault status of the default persona.');
      consoleWarnSpy.mockRestore();
    });

    it('should log an error if persona to update is not found', () => {
      const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
      const fakePersona: Persona = { id: 'fake-id', name: 'Fake', systemPrompt: '', live2dModelUrl: '', isDefault: false };
      personaManager.updatePersona(fakePersona);

      expect(consoleErrorSpy).toHaveBeenCalledWith(`Persona with ID "${fakePersona.id}" not found.`);
      consoleErrorSpy.mockRestore();
    });
  });
  ...
  describe('deletePersona', () => {
    it('should delete a non-default persona', () => {
      ...
      expect(personaManager.getPersonas().find(p => p.id === personaToDelete.id)).toBeUndefined();
    });
    ...
    it('should not allow deleting the default persona', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('The default persona cannot be deleted.');
      consoleWarnSpy.mockRestore();
    });
